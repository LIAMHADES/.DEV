(function () {
  ...
  function showManageButton() {
    ...
    if (document.querySelector("[data-ares-cookie-preferences]")) return;
    if (!document.querySelector("style[data-ares-manage]")) {
      var ms = document.createElement("style");
      ms.setAttribute("data-ares-manage", "1");
      ms.textContent = ".ares-consent-manage{display:block;margin:0.4rem auto 0.6rem;padding:.25rem .6rem;background:transparent;color:rgba(240,242,245,.4);border:none;opacity:.55;transition:opacity .25s,color .25s;font-size:8px;cursor:pointer;font-family:JetBrains Mono,monospace;letter-spacing:.08em;text-transform:uppercase;text-align:center}.ares-consent-manage:hover{opacity:1;color:rgba(240,242,245,.8)}";
    ...
    if (footer) footer.appendChild(manage);
    else document.body.appendChild(manage);
  }
  ...
  function init() {
    window.aresAnalytics = { track: track, manageConsent: showConsent };
    bindTracking();
